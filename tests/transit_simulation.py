...
pts = np.column_stack((y_vis, z_vis))
tri = Delaunay(pts)
star_surface = ax1.tripcolor(y_vis, z_vis, tri.simplices, I_vis, 
                            cmap='plasma', vmin=0.9*I_vis.min(), vmax=I_vis.max(), 
                            shading='gouraud')

surface_lines_spherical(ax1, Re_eq, Rp_polar, lamda, i_s, 
                             num_meridians=12, num_parallels=5, 
                             line_color='black', line_alpha=0.6, line_width=1.0)
